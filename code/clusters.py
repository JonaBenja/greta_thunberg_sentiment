...
def preprocess(article):
    ...
    for s in processed_article.sentences:
        lemmas = [word.lemma.lower() for word in s.words if word.text.lower() not in stopwords]
        clean_lemmas = [lemma for lemma in lemmas if lemma not in string.punctuation]
...
"""
DUTCH WORD EMBEDDINGS FOR CLUSTERING
"""
...
"""
ITALIAN DOCUMENT KEYWORDS
"""

def preprocess(article):
    processed_article = nlp.process(article)
    all_lemmas = []
    stopwords = ['ad', 'al', 'allo', 'ai', 'agli', 'all', 'agl', 'alla', 'alle', 'con', 'col', 'coi', 'da', 'dal',
                 ...
                 'contro', 'io', 'tu', 'lui', 'lei', 'noi', 'voi', 'loro', 'mio', 'mia', 'miei', 'mie', 'tuo', 'tua',
                 'tuoi', 'tue', 'suo', 'sua', 'suoi', 'sue', 'nostro', 'nostra', 'nostri', 'nostre', 'vostro', 'vostra',
                 'vostri', 'vostre', 'mi', 'ti', 'ci', 'vi', 'lo', 'la', 'li', 'le', 'gli', 'ne', 'il', 'un', 'uno',
                 'una', 'ma', 'ed', 'se', 'perché', 'anche', 'come', 'dov', 'dove', 'che', 'chi', 'cui', 'non', 'più',
                 'quale', 'quanto', 'quanti', 'quanta', 'quante', 'quello', 'quelli', 'quella', 'quelle', 'questo',
                 'questi', 'questa', 'queste', 'si', 'tutto', 'tutti', 'a', 'c', 'e', 'i', 'l', 'o', 'ho', 'hai', 'ha',
                 'abbiamo', 'avete', 'hanno', 'abbia', 'abbiate', 'abbiano', 'avrò', 'avrai', 'avrà', 'avremo',
                 'avrete', 'avranno', 'avrei', 'avresti', 'avrebbe', 'avremmo', 'avreste', 'avrebbero', 'avevo',
                 'avevi', 'aveva', 'avevamo', 'avevate', 'avevano', 'ebbi', 'avesti', 'ebbe', 'avemmo', 'aveste',
                 ...
                 'faremmo', 'fareste', 'farebbero', 'facevo', 'facevi', 'faceva', 'facevamo', 'facevate', 'facevano',
                 'feci', 'facesti', 'fece', 'facemmo', 'faceste', 'fecero', 'facessi', 'facesse', 'facessimo',
                 'facessero', 'facendo', 'sto', 'stai', 'sta', 'stiamo', 'stanno', 'stia', 'stiate', 'stiano', 'starò',
                 'starai', 'starà', 'staremo', 'starete', 'staranno', 'starei', 'staresti', 'starebbe', 'staremmo',
                 'stareste', 'starebbero', 'stavo', 'stavi', 'stava', 'stavamo', 'stavate', 'stavano', 'stetti',
                 'stesti', 'stette', 'stemmo', 'steste', 'stettero', 'stessi', 'stesse', 'stessimo', 'stessero',
                 'stando', 'essere', 'avere', 'fare']
    for s in processed_article.sentences:
        lemmas = [word.lemma.lower() for word in s.words if word.text.lower() not in stopwords]
        clean_lemmas = [lemma for lemma in lemmas if lemma not in string.punctuation]
        all_lemmas.extend(clean_lemmas)
...
"""
ITALIAN WORD EMBEDDINGS FOR CLUSTERING
"""

print("loading")
fasttext_model  = KeyedVectors.load_word2vec_format("../data/embedding_models/ita_newscrawl_2019_1M.bin", binary=True)
print("done loading")
...
overview.to_csv ('../data/clusters/it_4_topic_clusters.tsv', sep='\t', index = False, header=True)